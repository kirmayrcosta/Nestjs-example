...
describe('Given UpdateQuoteToCurrencyUsecase', () => {
  ...
  it('When call to update quote to non existence currency by alias Should return Error', async () => {
    ...
    expect(updateQuoteToCurrencyUsecase.exec("BRL", "ACD", quote)).rejects.toThrow(new BadRequestException('Currency not found'));
  ...
  it('When call to update quote to non existence currency by alias Then return Error', async () => {
    ...
    expect(updateQuoteToCurrencyUsecase.exec("BRL", "ABD", quote)).rejects.toThrow(new BadRequestException('Quote already exists'));
