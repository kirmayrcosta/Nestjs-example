...
describe('Given AddQuoteToCurrencyUseCase', () => {
  ...
  it('When call to add quote to noexistence currency Should return Error', async () => {
    ...
    expect(addQuoteToCurrencyUseCase.exec('ABC', input)).rejects.toThrow(new BadRequestException('Currency not found'));
    ...
    it('When call to add existence quote currency Should return Error', async () => {
        ...
        expect(addQuoteToCurrencyUseCase.exec('ABC', input)).rejects.toThrow(new BadRequestException('Quote already exists'));
